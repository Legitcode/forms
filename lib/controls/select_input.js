...
var SelectInput = (function (_React$Component) {
  ...
  _createClass(SelectInput, [{
    ...
    value: function componentDidMount() {
      ...
      _react2['default'].findDOMNode(this.refs.select).addEventListener('click', function (ev) {
        ev.stopPropagation();
        _this.toggleDropDown();
      });
    }
    ...
    value: function changeSelected(event) {
      var valueElement = event.target.firstChild;
    ...
    value: function render() {
      ...
      var options = this.props.options.map(function (option) {
        return _react2['default'].createElement(
          'a',
          { href: option.url, key: 'selectOption-' + option.value, onClick: _this2.changeSelected.bind(_this2), className: 'select-option' },
          _react2['default'].createElement(
            'span',
            { className: 'select-value', value: option.value },
            option.displayValue
          )
        );
      });
