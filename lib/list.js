...
var List = (function (_React$Component) {
  ...
  _createClass(List, [{
    ...
    value: function serialize(withAttrs) {
      ...
      ary = _underscore2['default'].compact(ary);

      if (_underscore2['default'].size(ary) > 0) {
        currentObjects = _utilsObj2['default'].clone(this.props.listItems);

        Object.keys(currentObjects).forEach(function (values, index) {
          Object.keys(ary[index]).forEach(function (key) {
            currentObjects[values][key] = ary[index][key];

            if (withAttrs) {
              var mergeableAttrs = {};
              mergeableAttrs[key] = _this4.formAttrs['' + key.split('-')[0]];
              currentObjects[values][key] = _underscore2['default'].extend(currentObjects[values][key], mergeableAttrs[key]);
            }
          });
        });
      }
    ...
    value: function generateChildren() {
      ...
      if (listItems) {
        children = Object.keys(listItems).map(function (key) {
